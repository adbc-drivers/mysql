...
const logSinkEnvVar = "ADBC_DRIVER_MYSQL_LOG_SINK"
...
func initLoggingFromEnv(db adbc.DatabaseLogging) {
	...
	case "tempfile":
		...
		f, err := os.CreateTemp("", "adbc-mysql.*.log")
		if err == nil {
			sink = f
		} else {
			fmt.Fprintf(os.Stderr, "[mysql] could not open temporary log file: %v", err)
		}
	}
